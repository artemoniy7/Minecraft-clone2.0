...
void renderInventory(int screenW, int screenH) {
    ...
    drawColorRectangle(
        ...
        static_cast<float>(INV_W),
        static_cast<float>(INV_H),
        screenW,
        screenH,
    ...
    constexpr float INVENTORY_SLOT_SPACING_X = 1.2f;
    constexpr float INVENTORY_SLOT_SPACING_Y = 1.2f;
    const int slotStepX = std::max(1, static_cast<int>(slotW * INVENTORY_SLOT_SPACING_X));
    const int slotStepY = std::max(1, static_cast<int>(slotH * INVENTORY_SLOT_SPACING_Y));
    ...
    for (int row = 0; row < listRowsVisible; ++row) {
        for (int col = 0; col < listCols; ++col) {
            ...
            const int y = listStartY + row * slotStepY;
    ...
    for (int row = 0; row < 3; ++row) {
        for (int col = 0; col < 9; ++col) {
            ...
            const int x = posX + static_cast<int>(8.0f * scaleX) + col * slotStepX;
            const int y = posY + static_cast<int>(58.0f * scaleY) + row * slotStepY;
            const int itemPadding = std::max(1, std::min(slotW, slotH) / 8);
            const int itemSize = std::max(8, std::min(slotW, slotH) - itemPadding * 2);
            const auto it = itemTypes.find(itemId);
            const bool isBlockItem = (it == itemTypes.end()) || it->second.isBlock;
...
void renderSingleBlockModel(int blockType) {
    // Создаём временный VAO для одного блока
    ...
    // Если модель ещё не создана для этого типа блока
    if (singleBlockVAO.find(blockType) == singleBlockVAO.end()) {
        std::vector<float> vertices;
        
        // Координаты куба от -0.5 до 0.5
        float size = 1.0f;
        float offset = 0.0f;
        
        // Генерируем вершины для куба (все 6 граней)
        // LEFT FACE
        addFaceToVertices(vertices, 
            glm::vec3(-size/2, -size/2, -size/2), glm::vec3(-size/2, -size/2, size/2),
            glm::vec3(-size/2, size/2, size/2), glm::vec3(-size/2, size/2, -size/2),
            glm::vec3(-1,0,0), 1.0f/3.0f);
...
void addFaceToVertices(std::vector<float>& verts, 
    glm::vec3 v1, glm::vec3 v2, glm::vec3 v3, glm::vec3 v4,
    glm::vec3 normal, float uOffset) {
// Правильные UV координаты для каждого угла квадрата
// Порядок обхода: v1 -> v2 -> v3 -> v4 -> v1
float uvs[] = {
0.0f, 1.0f,  // v1
1.0f, 1.0f,  // v2
1.0f, 0.0f,  // v3
...
// Рисуем два треугольника: (v1,v2,v3) и (v1,v3,v4)
...
float u = uvs[idx * 2];
float v = uvs[idx * 2 + 1];
// Применяем смещение для выбора нужной текстуры из стрипа
verts.push_back(uOffset + u * (1.0f/3.0f));
verts.push_back(v);
