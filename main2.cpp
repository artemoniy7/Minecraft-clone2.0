...
void renderInventory(int screenW, int screenH) {
    ...
    for (int row = 0; row < listRowsVisible; ++row) {
        for (int col = 0; col < listCols; ++col) {
            ...
            const int itemPadding = isBlockItem ? std::max(1, static_cast<int>(8.0f * scaleX)) : std::max(1, slotW / 6);
    ...
    if (scrollTex != 0) {
        const int trackX = posX + static_cast<int>(174.0f * scaleX) + static_cast<int>(2.0f * scaleX);
